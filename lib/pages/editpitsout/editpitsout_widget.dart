...
import 'editpitsout_model.dart';
export 'editpitsout_model.dart';

class EditpitsoutWidget extends StatefulWidget {
  const EditpitsoutWidget({
    super.key,
    required this.eventid,
    required this.eventname,
    required this.pitscoutid,
    String? pictureurl,
  }) : pictureurl = pictureurl ?? '';

  final int? eventid;
  final String? eventname;
  final int? pitscoutid;
  final String pictureurl;

  @override
  State<EditpitsoutWidget> createState() => _EditpitsoutWidgetState();
}

class _EditpitsoutWidgetState extends State<EditpitsoutWidget> {
  late EditpitsoutModel _model;
  ...
  void initState() {
    super.initState();
    _model = createModel(context, () => EditpitsoutModel());

    // On page load action.
    SchedulerBinding.instance.addPostFrameCallback((_) async {
      _model.pictureurl = widget.pictureurl;
      safeSetState(() {});
    });
  ...
  Widget build(BuildContext context) {
    return FutureBuilder<List<PitscoutlistRow>>(
      future: PitscoutlistTable().querySingleRow(
      ...
      builder: (context, snapshot) {
        ...
        List<PitscoutlistRow> editpitsoutPitscoutlistRowList = snapshot.data!;

        final editpitsoutPitscoutlistRow =
            editpitsoutPitscoutlistRowList.isNotEmpty
                ? editpitsoutPitscoutlistRowList.first
                : null;
                                  ...
                                  children: [
                                      ...
                                      child: SizedBox(
                                        ...
                                        child: TextFormField(
                                          controller:
                                            ...
                                            text: editpitsoutPitscoutlistRow
                                                ?.teamnumber,
                                          ),
                                    ...
                                    Padding(
                                      padding: const EdgeInsetsDirectional.fromSTEB(
                                          0.0, 20.0, 0.0, 0.0),
                                      child:
                                          FutureBuilder<List<PitscoutlistRow>>(
                                        future:
                                            PitscoutlistTable().querySingleRow(
                                          queryFn: (q) => q.eqOrNull(
                                            'id',
                                            widget.pitscoutid,
                                          ),
                                        ...
                                        builder: (context, snapshot) {
                                          ...
                                          List<PitscoutlistRow>
                                              buttonPitscoutlistRowList =
                                              snapshot.data!;

                                          final buttonPitscoutlistRow =
                                              buttonPitscoutlistRowList
                                                      .isNotEmpty
                                                  ? buttonPitscoutlistRowList
                                                      .first
                                          ...
                                          return FFButtonWidget(
                                            onPressed: () async {
                                              await PitscoutlistTable().update(
                                                data: {
                                                  'teamnumber': _model
                                                      .teamnumTextController
                                                      .text,
                                                  'expautoendgamepos':
                                                      _model.autoendgameValue,
                                                  'expteleopendgamepos':
