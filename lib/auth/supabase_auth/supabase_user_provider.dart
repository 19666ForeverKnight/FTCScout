...
class FTCScoutSupabaseUser extends BaseAuthUser {
  FTCScoutSupabaseUser(this.user);
  User? user;
  @override
...
Stream<BaseAuthUser> fTCScoutSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = FTCScoutSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
